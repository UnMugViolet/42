	...
	while (++i < map_rows(engine->data.map.array))
	{
		...
		while (++j < map_max_len(engine->data.map.array))
		{
			if (engine->data.map.array[i][j] == '1')
				ft_draw_square(engine, (t_point){j * engine->data.tile,  i * engine->data.tile}, engine->data.tile, WHITE);
			else if (engine->data.map.array[i][j] != ' ')
				ft_draw_square(engine, (t_point){j * engine->data.tile,  i * engine->data.tile}, engine->data.tile, RED);
		}
	} */
/* 	t_point tmp;

	tmp.x = engine->data.player.pos.x * engine->data.tile;
	tmp.y = engine->data.player.pos.y * engine->data.tile;
